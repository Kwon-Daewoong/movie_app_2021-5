...
>  - 기본 리스트 컴포넌트
>   - 여러개의 컴포넌트를 렌더링하는 예제를 numbers 배열을 props로 받아서 순서 없는 엘리먼트 리스트를 출력하는 컴포넌트로 리팩토링 하려면 key를 넣어야한다는 오류가 발생
   3. key
>  - Key는 React가 어떤 항목을 변경, 추가 또는 삭제할지 식별하는 것을 도움 됨
>  - key는 엘리먼트에 안정적인 고유성을 부여하기 위해 배열 내부의 엘리먼트에 지정함
>  - 일반적으로 항목이 고정적일 경우 배열의 index값 사용함
>  - 항목의 순서가 바뀔 수 있는 경우 key에 index를 사용하는 것은 권장 하지않음 , 성능이 저하되거나 컴포넌트의 state와 관련된 문제가 발생할 수 있음.
...
>  - codesandbox는 create-react-app으로 생성된 프로젝트와 동일 환경에서 테스트가 가능
>  - cdn방식으로 간편하게 테스트 할 수 있도록 html코드 제공
>  - REACT 문서가 어렵게 느껴진다면 tania rascia 가 작성한 REACT 개요를 먼저 학습
>  - 개발을 통해 react를 학습하고자 한다면 자습서를 추천
